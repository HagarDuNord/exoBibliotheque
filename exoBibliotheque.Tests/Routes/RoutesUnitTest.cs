...
namespace exoBibliotheque.Tests.Routes
{
    [TestClass]
    public class RoutesUnitTest
    {
        ...
        [TestMethod]
        public void TestRoute_Ajouter_Livre()
        {
            RouteData routeData = DefinirUrl("~/Ajouter/Livre");

            Assert.IsNotNull(routeData);
            Assert.AreEqual("Ajouter", routeData.Values["controller"]);
            Assert.AreEqual("Livre", routeData.Values["action"]);
